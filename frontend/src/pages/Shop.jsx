...
const ProductCard = ({ product, onAddToCart }) => {
  ...
  return (
    <div className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <div className="relative h-52 overflow-hidden bg-white p-4 flex items-center justify-center border-b border-slate-100">
        ...
        />
        <div className="absolute left-4 top-4 inline-flex rounded-full bg-white/90 px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-700 shadow-sm border border-slate-200">
          {product.is_mandatory ? 'Required' : 'Optional'}
        </div>
        {discount > 0 && (
          <div className="absolute right-4 top-4 inline-flex rounded-full bg-rose-600 px-3 py-1 text-[11px] font-black uppercase tracking-wider text-white shadow-md">
            {discount}% OFF
          </div>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mb-4 mt-5 flex items-center gap-2">
          <select
            ...
            className="rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
          >
            {['S', 'M', 'L', 'XL', 'XXL'].map((s) => (
          ...
          <div className="flex items-center rounded-xl border border-slate-300">
            ...
            >-</button>
            <span className="min-w-10 px-2 text-center text-sm font-semibold text-slate-900">{qty}</span>
            <button
        ...
        <div className="mt-auto flex items-end justify-between border-t border-slate-100 pt-4">
          <div className="flex flex-col">
            ...
            <button onClick={() => setShowHistory(true)} className="mt-1 text-left text-[11px] font-semibold text-slate-400 underline transition hover:text-indigo-600">
              View price history
            </button>
          </div>
          <button
            ...
            className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-slate-300"
          >
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.3em] text-indigo-600">Shop uniforms</p>
          <h1 className="mt-3 text-4xl font-black text-slate-900">School Uniforms</h1>
          <p className="mt-3 text-base text-slate-500">Welcome{user ? `, ${user.name}` : ''}. Choose a grade and gender to view the exact catalog.</p>
        </div>
        <button
          type="button"
              ...
              {['Male', 'Female'].map((g) => (
                <button
                  ...
                  className={`rounded-xl px-5 py-2 text-sm font-semibold transition ${
                    gender === g ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'
                  }`}
                >
                  {g}
                </button>
      ...
      {recommendations.length > 0 && (
        <div className="mb-10">
          <div className="mb-4 flex items-center gap-3">
            ...
            <span className="rounded-full bg-indigo-100 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-indigo-700 border border-indigo-200">Based on past orders</span>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4">
            {recommendations.map((rec) => {
              ...
              return (
                <div key={rec.id} className="min-w-[220px] flex-shrink-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md">
                  <div className="relative mb-3 h-32 overflow-hidden rounded-xl bg-white p-2 flex items-center justify-center border border-slate-50">
                    <img src={resolveImageUrl(rec.image_url, rec.name)} alt={rec.name} onError={(e) => attachFallback(e, rec.name)} className="h-full w-full object-contain" />
                    {disc > 0 && (
