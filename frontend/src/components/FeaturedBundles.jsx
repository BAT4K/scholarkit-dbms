  ...
  return (
    <div className="mb-12">
      <div className="flex items-center gap-3 mb-6">
        <h2 className="text-2xl font-extrabold text-gray-900">1-Click Uniform Bundles</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {bundles.map((bundle, idx) => (
          <div key={idx} className="bg-gradient-to-br from-blue-50 to-indigo-50/50 p-6 rounded-2xl shadow-sm border border-blue-100 relative overflow-hidden flex flex-col group hover:shadow-md transition">
              ...
              <ul className="space-y-3 mb-6">
                {bundle.bundle_contents.required.map((item, i) => (
                  <li key={i} className="flex flex-start items-center text-sm font-medium text-gray-700">
                    ...
                    <span><span className="font-bold text-gray-900">{item.qty}x</span> {item.item}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="mt-auto pt-5 border-t border-blue-200/60 flex justify-between items-end">
              <div className="flex flex-col">
                ...
                <span className="text-2xl font-black text-gray-900 leading-none">₹{bundle.total_price}</span>
              </div>
              <button 
                onClick={() => onAddBundleToCart(bundle)}
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-xl shadow-lg shadow-blue-600/20 transition-all active:scale-95"
              >
                Add to Cart
              </button>
            </div>
